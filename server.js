...
let app = http.createServer((req, res) => {
  ...
  if (url == "/webp") {
    return req.on("end", () => {
      ...
      let log = `${date.getMonth() + 1}/${date.getDate()} ${date.getHours()}:${date.getMinutes()}: - ${params.get(
        "title"
      )} ${parseInt(params.get("cut"))} ${params.get("duration")} ${params.get("webpGif")} - ${ip}`;
      if (debug) console.log(log);
      exec(`echo "${log}" >> log.txt`, () => {});

      let dir = `temp/${crypto
        .createHash("sha256")
        .update(ip + params.get("time"))
        .digest("hex")}`;
      let cut = parseInt(params.get("cut"));
      let duration = parseInt(params.get("duration"));
      let lastCut = cut + duration - 1;
      let filename = `${params.get("trimName")}_${cut.toString().padStart(5, "0")}-${lastCut
        .toString()
        .padStart(5, "0")}.${params.get("webpGif")}`;
...
function imagesDownload(dir, params, res, form) {
  ...
  for (let i = 0; i < duration; i++) {
    ...
    promises.push(
      download(new URL(`${cloud}/${params.get("title")}/${filename}`).href, `${dir}/${filename}`, res, form)
    );
  }
...
function ffmpeg(dir, filename, res, form) {
  return new Promise((resolve, reject) => {
    ...
    let p = exec(
      `ffmpeg -framerate 12 -pattern_type glob -i "${dir}/*.jpg" ${command} "${dir}/${filename}" -progress pipe:1`,
      (err) => {
        if (err) reject(err);
        else {
